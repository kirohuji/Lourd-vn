...
export function ProjectLayout() {
    ...
    // 判断当前路由是否需要显示右侧面板（只有章节详情页面才需要）
    const shouldShowRightPanel = chapterId && !isNaN(chapterId);
    // 判断当前路由是否不需要右侧面板（项目资源管理、项目策划等）
    const isNonChapterPage =
    ...
    return (
        <div className='flex h-screen flex-col'>
            ...
            <div className='relative flex flex-1 overflow-hidden'>
                ...
                {/* 右侧：章节属性面板 - 只在章节详情页面显示 */}
                {!isNonChapterPage && (
                    <>
                        <div
                            className={cn(
                                'relative transition-all duration-300 ease-in-out overflow-hidden',
                                shouldShowRightPanel && rightSidebarOpen ? 'w-80' : 'w-0',
                            )}
                        >
