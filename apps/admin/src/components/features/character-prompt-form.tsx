...
export function CharacterPromptForm({
    ...
    useEffect(() => {
        if (isEditMode && characterPrompt) {
            ...
            setPrompt(characterPrompt.basicPrompt);
            ...
            <DialogContent className='max-w-5xl max-h-[90vh] overflow-auto'>
                ...
                <Tabs defaultValue='basic' className='w-full'>
                    <TabsList className='grid w-full grid-cols-3'>
                        ...
                        <TabsTrigger value='variants' disabled={!isEditMode || !characterPrompt?.id}>
                            变体管理
                        </TabsTrigger>
                    ...
                    <TabsContent value='variants' className='space-y-4'>
                        {isEditMode && characterPrompt?.id && (
                            <PromptVariantManager
                                characterPromptId={characterPrompt.id}
                                basicPrompt={prompt}
                                disabled={createMutation.isPending || updateMutation.isPending}
                            />
                        )}
