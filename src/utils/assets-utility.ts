...
export async function defineAssets() {
    try {
        ...
        // 从 IndexedDB 生成动态 manifest
        const dynamicManifest = await manifestManager.generateManifestFromIndexedDB(manifest);
        ...
        console.log('Assets initialized with dynamic manifest from IndexedDB');
    } catch (error) {
        console.error('Failed to initialize assets with IndexedDB resources:', error);

        // 如果 IndexedDB 初始化失败，回退到原始 manifest
